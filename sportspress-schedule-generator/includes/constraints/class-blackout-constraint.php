...
class SPSG_Blackout_Constraint extends SPSG_Abstract_Constraint
{
    ...
    private function track_missed_game($game, $blackout_date, $config)
    {
        ...
        $this->log(sprintf('Tracked makeup game for %s vs %s on %s',
            $game->home_team->name,
            $game->away_team->name,
            $blackout_date->format('Y-m-d')
        ));
    ...
    public function schedule_makeup_games($schedule, $config)
    {
        ...
        foreach ($this->makeup_games as $makeup_key => $makeup_game) {
            ...
            if ($makeup_date) {
                ...
                $this->log(sprintf('Scheduled makeup game for %s on %s',
                    $makeup_key,
    ...
    private function get_alternative_days($original_day, $playing_days)
    {
        ...
        return array_filter($playing_days, function ($day) use ($original_day) {
