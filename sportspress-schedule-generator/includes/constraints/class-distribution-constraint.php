...
class SPSG_Distribution_Constraint extends SPSG_Abstract_Constraint {
    ...
    public function validate($game, $schedule, $config) {
        ...
        if ($cost > 0) {
            $this->log(sprintf('Distribution violation cost: %.2f for game %s vs %s on %s %s', 
                $cost,
                $game->home_team->name,
                $game->away_team->name,
                $game->date,
    ...
    private function calculate_day_distribution_cost($game, $schedule, $config) {
        ...
        $game_day = $game_date->format('l');
        
        // Get current distribution for both teams
        $home_team_distribution = $this->get_team_day_distribution($game->home_team->id, $schedule);
        $away_team_distribution = $this->get_team_day_distribution($game->away_team->id, $schedule);
        ...
        $home_cost = $this->calculate_team_day_cost($game->home_team->id, $game_day, $home_team_distribution, $target_ratios, $config);
        $cost += $home_cost;
        
        // Calculate cost for away team
        $away_cost = $this->calculate_team_day_cost($game->away_team->id, $game_day, $away_team_distribution, $target_ratios, $config);
        $cost += $away_cost;
    ...
    private function calculate_time_slot_distribution_cost($game, $schedule, $config) {
        ...
        $home_team_slots = $this->get_team_time_slot_distribution($game->home_team->id, $schedule);
        $away_team_slots = $this->get_team_time_slot_distribution($game->away_team->id, $schedule);
     ...
     */
    private function calculate_team_day_cost($team_id, $game_day, $current_distribution, $target_ratios, $config) {
    ...
    private function get_all_teams_from_schedule($schedule) {
        ...
        foreach ($schedule as $game) {
            $teams[$game->home_team->id] = true;
            $teams[$game->away_team->id] = true;
        }
