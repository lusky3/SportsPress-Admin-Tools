...
require_once __DIR__ . '/../includes/class-updater-release.php';
...
foreach ( array( 'v1.1.0', '1.1.0', 'v1.1', 'v10.20.30' ) as $tag ) {
	assert_test( $R::is_release_tag( $tag ), "'{$tag}' is a release" );
}
foreach ( array( 'v1.1.0-rc5', 'v1.1.0-beta', 'v1.1.0+build.7', 'nightly', '', 'v1', 'v1.2.3.4' ) as $tag ) {
	assert_test( ! $R::is_release_tag( $tag ), "'{$tag}' is not a release" );
}
...
assert_test( $R::is_trusted_package( $good ), 'a release asset from this repository is accepted' );
assert_test(
	$R::is_trusted_package( 'https://objects.githubusercontent.com/lusky3/SportsPress-Admin-Tools/releases/download/v1.1.0/x.zip' ),
...
assert_test(
	! $R::is_trusted_package( 'https://github.com/someone-else/evil/releases/download/v1/x.zip' ),
	'another repository on the same host is refused'
);
assert_test( ! $R::is_trusted_package( str_replace( 'https://', 'http://', $good ) ), 'plain http is refused' );
assert_test( ! $R::is_trusted_package( 'https://github.evil.com/lusky3/SportsPress-Admin-Tools/releases/download/v1/x.zip' ), 'a lookalike host is refused' );
assert_test( ! $R::is_trusted_package( 'https://github.com/lusky3/SportsPress-Admin-Tools/archive/main.zip' ), 'a non-release path on the right repository is refused' );
assert_test( ! $R::is_trusted_package( '' ), 'an empty URL is refused' );
assert_test( ! $R::is_trusted_package( 'not a url' ), 'a malformed URL is refused' );
...
assert_test( '1.0.5' === $R::entry_for( $manifest, 'sportspress-admin-tools' )['version'], 'each plugin gets its own version out of a single release' );
assert_test( '1.1.0' === $R::entry_for( $manifest, 'sportspress-player-tools' )['version'], '  even when they differ within that release' );
assert_test( null === $R::entry_for( $manifest, 'sportspress-score-sheets' ), 'a plugin the release predates is silence, not an error' );
assert_test( null === $R::entry_for( $manifest, 'sportspress-broken' ), 'an entry with no version is ignored' );
assert_test( null === $R::entry_for( array(), 'anything' ), 'an empty manifest offers nothing' );
...
assert_test( $good === $R::asset_url( $assets, 'sportspress-player-tools.zip' ), 'the asset URL comes from the release listing' );
assert_test( '' === $R::asset_url( $assets, 'sportspress-score-sheets.zip' ), 'an asset that failed to upload reads as absent, not as a URL that 404s mid-upgrade' );
assert_test( '' === $R::asset_url( $assets, 'hijacked.zip' ), 'an asset pointing off-host is discarded even though the release lists it' );
assert_test( '' === $R::asset_url( array(), 'anything.zip' ), 'a release with no assets offers nothing' );
