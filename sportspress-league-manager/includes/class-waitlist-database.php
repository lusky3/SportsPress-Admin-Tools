...
class SPLM_Waitlist_Database {
	...
	// 1.2.0 adds offer_message, the same way.
	...
	public static function create_table(): bool {
		...
		$sql = "CREATE TABLE {$table} (
			...
			offer_message varchar(500) NOT NULL DEFAULT '',
	 ...
	 *
	 * Status alone is not always enough to identify what was read, because a
	 * status can come back around: cancel returns an offer to `queued` and a
	 * re-offer makes it `offered` again, with a new token and a new deadline.
	 * An expiry or unwind still in flight against the FIRST offer would match
	 * that replacement on status alone and expire it, or strip the token and
	 * deadline off an invitation that had only just gone out.
	 *
	 * So an offered row is identified by its claim token as well. The token is
	 * regenerated by every offer and is unique, which makes it the offer's
	 * identity — the same job a revision column would do, without a schema
	 * change. Rows with no token (queued, and never-offered) pass null and are
	 * identified by status alone, which is sufficient: there is no earlier
	 * offer left in flight to be confused with.
	 *
	 * @param int         $id              Row id.
	 * @param string      $expected_status Status the caller based its decision on.
	 * @param array       $data            Columns to write.
	 * @param string|null $expected_token  Claim token the caller read, when the
	 *                                     row had one.
	 * @return bool True when this update applied; false when the row had
	 *              already moved on, or the write failed.
	 */
	public static function update_if_status( int $id, string $expected_status, array $data, ?string $expected_token = null ): bool {
		global $wpdb;
		$data['updated_at'] = self::now();
